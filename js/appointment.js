$(document).ready(function(){
	...
	$('#createAppointment').click(function(){
		$('#appointmentModal').modal({
			...
			displaySlots($('#consultant_name').val(), $('#appointment_date').val());
		});
		...
		$.ajax({
			...
			success:function(data){
				$("#appointmentDetails").on("shown.bs.modal", function () {
					$('#a_client').html(data.client_name);
					$('#a_consultant').html(data.consultant_name);
...
function displaySlots(consultantId, appointmentDate) {
	var action = 'getSlots';
	$.ajax({
		url:'appointment_action.php',
		method:"POST",
		data:{consultantId:consultantId, appointmentDate:appointmentDate, action:action},
		success:function(data){
